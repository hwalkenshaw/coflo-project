...
import { PersonDto, updatePerson } from '../../../services/people.service'
import SlideOverForm from '../../common/SlideOverForm'

type Props = { open: boolean; person: PersonDto | null; onClose: () => void; onUpdated: () => void }
...
export default function PeopleEditDialog({ open, person, onClose, onUpdated }: Props) {
  ...
  useEffect(() => {
    if (person) {
      reset({ firstName: person.firstName, lastName: person.lastName, dateOfBirth: person.dateOfBirth.substring(0, 10) })
    }
  }, [person])
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      await updatePerson(person.id, formattedValues)
      reset()
      onUpdated()
      onClose()
    } catch (error: any) {
      ...
      if (error.response?.status === 400 && error.response?.data?.errors) {
        ...
        setApiError(error.response?.data?.message || 'Failed to update person. Please try again.')
      }
    }
  })
